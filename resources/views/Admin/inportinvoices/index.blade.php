...
@section('content')
        ...
        <div class="page-title">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-left">
                    <li class="breadcrumb-item"><a href="#">Quản lý hóa đơn nhập</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Dánh sách hóa đơn nhập</li>
                </ol>
            </nav>
        </div>
        <section class="section">
            <div class="card">
                <div class=" card-header bg-white">
                    <u style="color: #0dcaf0; cursor: pointer;" class="font-italic"> Bảng danh sách hóa đơn nhập</u>
                </div>
                <div class="mb-3 bg-white">
                    <a href="{{ route('admin.importinvoice.create') }}">
                        <button type="button" class="ml-4 btn btn-success"><i class="mr-2  fas fa-plus"></i>Thêm hóa đơn nhập
                        </button>
                    </a>
                   
                    @if (Session::has('error-importinvoice_delete'))
                        <div style="margin: 0 15px;" class="alert alert-danger" role="alert">
                            {{ Session()->get('error-importinvoice_delete') }}
                        </div>
                    @endif
                </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($inportInvoice as $c)
                                <tr>
                                    ...
                                    <td>
                                        <a class="btn-trash"
                                            href="{{ route('admin.importinvoice.edit', ['importinvoice' => $c->id]) }}"><i
                                    ...
                                    <td>
                                        <a class="btn-delete" href="" data-id="{{ $c->id }}" data-toggle="modal"
                                            data-target="#delete-importinvoice"><i
    ...
    <div class="modal fade" id="delete-importinvoice" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Sau khi xóa bạn không thể khôi phục lại !
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
                    <button id="deleted-importinvoice" type="button" class="btn btn-danger">Đồng ý</button>
                </div>
            </div>
        </div>
    </div>
...
@section('script')
    <script>
        $(document).ready(function() {
            ...
            var getId;
            var btnDelete = document.querySelector('#deleted-importinvoice');
            var formDelete = document.querySelector('.trash-form');
            $('#delete-importinvoice').on('show.bs.modal', function(event) {
                var button = $(event.relatedTarget)
                getId = button.data('id')
            })

            btnDelete.onclick = function() {
                formDelete.action = '/admin/importinvoice/' + getId;
                formDelete.submit();
            }
