        ...
        <div class="page-title">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-left">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Danh sách sản phẩm đã xóa</li>
                </ol>
            </nav>
        </div>
        <section class="section">
            <div class="card">
                <div class=" card-header bg-white">
                    <u style="color: #0dcaf0; cursor: pointer;" class="font-italic"> Bảng danh sách sản phẩm đã
                        xóa</u>
                </div>
                <div class="mb-3 bg-white">
                    <a href="{{ route('admin.product.index') }}">
                        <button type="button" class="ml-4 btn btn-success"><i class="mr-2  far fa-list-alt"></i><i>Danh sách
                                sản phẩm</i></button>
                    </a>
                        ...
                        <tbody>
                            ...
                            @foreach ($products as $p)
                                <tr>
                                    <td>{{ $i++ }}</td>
                                    <td>{{ $p->name }}</td>
                                    <td ><img style="width: 200px" src="{{ asset('upload/image/product/'.$p->image) }}" alt="Ảnh sản phẩm"> </td>

                                    <td>
                                        <a class="btn-trash"
                                            href="{{ route('admin.product.untrash', ['id' => $p->id]) }}"><i
    ...
    <script>
        $(document).ready(function() {
            ...
            btnDelete.onclick = function() {
                formDelete.action = '/admin/product/' + getId;
                formDelete.submit();
            }
